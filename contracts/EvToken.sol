...
/// @title EvToken
///
/// @dev This is the contract for the EvryLoans utillity token usd.
///
...
contract EvToken is AccessControl, ERC20("EVRY USD", "evUSD") {
  ...
  event Paused(address evryloansAddress, bool isPaused);
  ...
  modifier onlyWhitelisted() {
    require(whiteList[msg.sender], "EvUSD: EvryLoans is not whitelisted");
    _;
  }
  ...
  function mint(address _recipient, uint256 _amount) external onlyWhitelisted {
    require(!blacklist[msg.sender], "EvUSD: EvryLoans is blacklisted.");
    uint256 _total = _amount.add(hasMinted[msg.sender]);
    require(_total <= ceiling[msg.sender],"EvUSD: EvryLoans's ceiling was breached.");
    require(!paused[msg.sender], "EvUSD: user is currently paused.");
  ...
  /// This function reverts if the caller does not have the admin role.
  function pauseEvryLoans(address _toPause, bool _state) external onlySentinel {
